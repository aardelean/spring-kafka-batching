...
public class TestKafkaFactory<K, S, R> implements TestRule{
    ...
    public KafkaTemplate<K, S> createProducer(Class producerSerializer, KafkaListenerEndpointRegistry kafkaListenerEndpointRegistry) throws Exception {
        ...
        senderProperties.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, Serdes.String().serializer().getClass().getName());
    ...
    public BlockingQueue<ConsumerRecord<K, R>> createConsumer(Class consumerDeserializer) throws Exception {
        ...
        consumerProperties.put(ConsumerConfig.CLIENT_ID_CONFIG, this.getClass().getPackageName());
        consumerProperties.put(ConsumerConfig.GROUP_ID_CONFIG, this.getClass().getPackageName());
        consumerProperties.put("application.id", this.getClass().getPackageName());
    ...
    public ConsumerRecord<K, R> sendAndReceive(K key, S dataToSend, KafkaListenerEndpointRegistry kafkaListenerEndpointRegistry,
                                               Class producerSerializer,
                                               Class consumerDeserializer) throws Exception {
        createProducer(producerSerializer, kafkaListenerEndpointRegistry).send(sendTopic, key, dataToSend);
    ...
    public Statement apply(Statement base, Description description) {
        embeddedKafka = new KafkaEmbedded(2, true, sendTopic, receiveTopic);
